...
export class TypeService {
  ...
  create(createTypeDto: CreateTypeDto, companyId: string) {
    return this.prisma.companyTaskType.create({
      data: {
        ...createTypeDto,
        company: {
          connect: {
  ...
  findAll(companyId: string) {
    return this.prisma.companyTaskType.findMany({
      where: {
        companyId: companyId,
        deleted: false,
      },
  ...
  async findOne(typeId: string, companyId: string) {
    const type = await this.prisma.companyTaskType.findFirst({
      where: {
        id: typeId,
        companyId: companyId,
        deleted: false,
  ...
  async update(
    ...
    return this.prisma.companyTaskType.update({
      where: {
        id: typeId,
      },
      data: updateTypeDto,
