...
export class TypeService {
  ...
  create(createTypeDto: CreateTypeDto, companyId: string) {
    return this.prisma.companyFinanceType.create({
      data: {
        ...createTypeDto,
        company: {
          connect: {
  ...
  findAll(companyId: string) {
    return this.prisma.companyFinanceType.findMany({
      where: {
        companyId: companyId,
        deleted: false,
      },
  ...
  async findOne(typeId: string, companyId: string) {
    const type = await this.prisma.companyFinanceType.findFirst({
      where: {
        id: typeId,
        companyId: companyId,
        deleted: false,
  ...
  async update(
    ...
    return this.prisma.companyFinanceType.update({
      where: {
        id: typeId,
      },
      data: updateTypeDto,
